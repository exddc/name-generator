...
from . import jobs
from .queue import q_orchestrator, redis_conn
from .groq_client import get_suggestions_from_llm

BACKEND_PORT = int(environ.get("BACKEND_PORT", 8000))
...
def suggest(suggest_request: SuggestRequest):
    ...
    try:
        ...
        suggestions_list = get_suggestions_from_llm(user_input)
...
def suggest_stream(query: str):
    request_id = str(uuid.uuid4())
    channel = f"sse:{request_id}"
    q_orchestrator.enqueue(jobs.orchestrate_suggest_and_check, request_id, query, job_timeout=180)
